              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align={typeof row.image == 'number' ? 'right' : 'left'}><Avatar alt={row.firstName} src={row.image} sx={{ bgcolor: indigo[50] }} /></TableCell>
                    <TableCell align={typeof row.firstName == 'number' ? 'right' : 'left'}>{row.firstName}</TableCell>
                    <TableCell align={typeof row.lastName == 'number' ? 'right' : 'left'}>{row.lastName}</TableCell>
                    <TableCell align={typeof row.username == 'number' ? 'right' : 'left'}>{row.username}</TableCell>
                    <TableCell align={typeof row.email == 'number' ? 'right' : 'left'}>{row.email}</TableCell>
                    <TableCell align={typeof row.phone == 'number' ? 'right' : 'left'}>{row.phone}</TableCell>
                    <TableCell align={typeof row.bankNumber == 'number' ? 'right' : 'left'}>{row.bankNumber}</TableCell>
                    <TableCell align={typeof row.macAddress == 'number' ? 'right' : 'left'}>{row.macAddress}</TableCell>
                  </TableRow>
                );
              })}
              {emptyRows > 0 && (
